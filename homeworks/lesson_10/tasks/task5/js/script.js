...
if (confirm('Почати тестування?')) {
  ...
  function getPriceChangeCount(prices) {
    const priceChanges = prices.reduce((count,el,index,arr)=> index > 0 && el !== arr[index-1] ? count+1 : count,0)
    return priceChanges
  }
  document.write(getPriceChangeCount(securitiesPrices));
}
